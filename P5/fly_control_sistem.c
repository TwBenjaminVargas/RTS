...
void* telemetry(void* arg) {
    ...
    while(atomic_load(&keepRunning)) {
        ...
        if (sig == SIGALRM)
        {
            int current_total = atomic_fetch_add(&telemetry_iterations, 1) + 1;
            printf("\rEnviando telemetria a tierra");
            int num_dots = current_total % 10;
            if (num_dots == 0 && current_total > 0) num_dots = 10;
...
int main()
{
    ...
    printf("Elija la politica de planificacion:\n");
